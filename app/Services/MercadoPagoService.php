...
class MercadoPagoService
{
    ...
    public function handlePayment(Request $request)
    {
        ...
        if ($payment->status === "approved") {
            ...
            return redirect()
                ->route('perfil')
                ->withSuccess(['payment' => "Gracias, {$name}. Hemos recibido tu pago de {$originalAmount}{$originalCurrency} en cual seria ($ {$amount} {$currency})."]);
        }

        return redirect()
            ->route('perfil')
            ->withErrors('No hemos podido recibir tu pago , favor intentalo nuevamente');
    }
