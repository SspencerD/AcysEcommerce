...
class PayUService
{
    ...
    public function handlePayment(Request $request)
    {
        ...
        $payment = $this->createPayment(
            ...
            $request->payu_installments,
        );

        if($payment->transactionResponse === "APPROVED"){
            ...
            return redirect()
            ->route('perfil')
            ->withSuccess(['payment' => "Gracias , {$name}.
             Hemos recibido tu pago de {$amount}{$currency}"]);
        }

        return redirect()
            ->route('perfil')
            ->withErrors('No hemos podido procesar tu pago , favor verifique los detalles e intenta nuevamente mas tarde.');
    }
    ...
    public function createPayment($value, $currency, $name, $email, $card, $cvc, $year, $month, $network, $installments, $paymentCountry = 'CO')
    {
